      ...
      <PageLayout>
        <div className="min-h-screen flex items-center justify-center">
          <div className="text-center">
            ...
            <div className="text-white text-xl">Loading Constitutional Authority Map...</div>
            <div className="text-gray-400 text-sm mt-2">Parsing {data?.metadata?.title || 'framework data'}</div>
          </div>
        </div>
      </PageLayout>
    );
      ...
      <PageLayout>
        <div className="min-h-screen flex items-center justify-center">
          <div className="text-center max-w-md">
            ...
            <div className="text-white text-xl mb-2">Failed to Load Data</div>
  ...
  return (
    <PageLayout>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {/* Framework Summary */}
        <div className="bg-gradient-to-r from-blue-950/50 via-purple-950/50 to-amber-950/50 border border-white/10 rounded-xl p-6 mb-8">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 text-center">
            <div className="p-4">
              <div className="text-3xl font-bold text-blue-400">
                {Object.keys(data.positions).length}
              </div>
              <div className="text-sm text-gray-400">Constitutional Positions</div>
            </div>
            <div className="p-4">
              <div className="text-3xl font-bold text-emerald-400">
                {Object.keys(data.cross_reference_indexes.by_power).length}
              </div>
              <div className="text-sm text-gray-400">Enumerated Powers</div>
            </div>
            <div className="p-4">
              <div className="text-3xl font-bold text-purple-400">
                {Object.keys(data.relationship_matrices.checks_matrix).length}
              </div>
              <div className="text-sm text-gray-400">Check Relationships</div>
            </div>
            <div className="p-4">
              <div className="text-3xl font-bold text-amber-400">
                {data.psv_framework.compound_psvs.length}
              </div>
              <div className="text-sm text-gray-400">Compound PSVs</div>
            </div>
          </div>
        </div>
        ...
        <div className="min-h-[600px]">
          ...
          {activeTab === 'compliance-test' && (
            <div className="max-w-4xl mx-auto">
              <div className="bg-black/80 backdrop-blur-md rounded-xl border border-white/10 p-6 mb-8">
                ...
                <div className="bg-gradient-to-r from-blue-950/50 to-purple-950/50 border border-blue-500/30 rounded-lg p-4 mb-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-6 gap-2 text-xs">
                    {data.compliance_tools.madisonian_test.steps.map((step, idx) => (
                      <div key={idx} className="bg-black/40 rounded-lg p-2 border border-white/10">
                        <div className="text-blue-400 font-semibold mb-1">Step {step.step}</div>
                        <div className="text-gray-300">{step.name}</div>
                      </div>
                    ))}
                  </div>
                </div>
        ...
        <div className="mt-12 text-center">
          <div className="bg-black/80 backdrop-blur-md rounded-xl border border-white/10 p-6 max-w-3xl mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-xs mb-4">
              <div className="bg-red-900/20 border border-red-500/30 rounded-lg p-3">
                ...
                <div className="text-gray-400">Direct usurpation or abdication of an EAP</div>
              </div>
              <div className="bg-amber-900/20 border border-amber-500/30 rounded-lg p-3">
                <div className="text-amber-400 font-semibold mb-1">ENABLING PSV</div>
                <div className="text-gray-400">Law/doctrine permitting improper delegation</div>
              </div>
              <div className="bg-purple-900/20 border border-purple-500/30 rounded-lg p-3">
                <div className="text-purple-400 font-semibold mb-1">SHIELDING PSV</div>
                <div className="text-gray-400">Doctrine blocking remedy for violations</div>
              </div>
            </div>
        ...
        <div className="mt-8">
          <div className="bg-black/60 border border-white/10 rounded-xl p-6 max-w-3xl mx-auto">
            <h4 className="text-white font-semibold mb-4 text-center">Constitutional Authority Derivation Hierarchy</h4>
              ...
              {Object.entries(data.hierarchy_tiers).map(([tierId, tier], idx) => (
                <div key={tierId} className="flex items-center gap-4">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="text-gray-500 text-xs">{tier.constraint}</div>
                  </div>
