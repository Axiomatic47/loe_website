// src/components/ImageEnhancedMarkdownRenderer.tsx - Enhanced with better debugging and path resolution
...
// Enhanced image path resolution with comprehensive debugging
const resolveImagePath = (src: any): string => {
  return enhancedResolveImagePath(src);
};

// Enhanced Image Component with better error handling and auto-fallback
const EnhancedImage: React.FC<{
  ...
  const handleImageError = async () => {
    ...
    if (import.meta.env.DEV && fallbackPaths.length > retryCount + 1) {
      ...
      const exists = await testImagePath(nextFallback);
      if (exists) {
        setCurrentSrc(nextFallback);
        setRetryCount(retryCount + 1);
        setImageError(false);
        console.log(`✅ Fallback successful: ${nextFallback}`);
        return;
      }
    }
  ...
  if (!currentSrc) {
    return (
      <div className="w-full h-48 bg-black/60 border border-yellow-500/50 rounded flex flex-col items-center justify-center text-yellow-300 p-4">
  ...
  if (imageError) {
    return (
      <div className="w-full h-48 bg-black/60 border border-red-500/50 rounded flex flex-col items-center justify-center text-red-300 p-4">
        <AlertCircle className="h-8 w-8 mb-2" />
        ...
        <div className="flex gap-2">
          ...
          {import.meta.env.DEV && retryCount < fallbackPaths.length - 1 && (
            <Button
              variant="outline"
              size="sm"
              className="bg-blue-900/30 border-blue-500/50 text-blue-300 hover:bg-blue-900/50"
        ...
        {import.meta.env.DEV && (
          <details className="mt-2 w-full">
            ...
            <pre className="text-xs mt-1 bg-black/40 p-2 rounded max-w-full overflow-auto">
              {JSON.stringify({
                original: src,
                resolved: currentSrc,
                retryCount,
                fallbacksAvailable: fallbackPaths.length
              }, null, 2)}
            </pre>
          </details>
        )}
    ...
    <div className="relative group">
      ...
      {caption && (
        <div className="p-3 bg-black/60">
          <p className="text-sm text-gray-200 text-center">{caption}</p>
        </div>
      )}
    </div>
  );
      ...
      <div className={cn(
        ...
        {filteredImages.map((image, index) => (
          <div key={`${position}-${index}`} className="relative group">
            <div className="bg-black/40 rounded-lg border border-white/20 overflow-hidden hover:border-white/30 transition-all duration-200 shadow-lg">
              <EnhancedImage
                src={image.src}
                alt={image.alt}
        ...
        >
          <div className="relative max-w-4xl max-h-[90vh] w-full" onClick={(e) => e.stopPropagation()}>
            <Button
              variant="outline"
              size="icon"
              className="absolute top-4 right-4 z-[10000] bg-black/60 text-white border-white/30 hover:bg-black/80"
              onClick={closeModal}
            >
              <X className="h-4 w-4" />
            </Button>

            <div className="bg-black/40 rounded-lg border border-white/20 overflow-hidden shadow-2xl">
              <EnhancedImage
      ...
      {inlineImages.map((image, index) => (
        <span key={`inline-${index}`} className="inline-block mx-2 my-1 relative group">
          <EnhancedImage
            src={image.src}
            alt={image.alt}
            className="inline h-8 w-auto rounded border border-white/30 hover:border-white/50 transition-colors cursor-pointer"
          />
        </span>
      ))}
    </>
...
// Replace your customComponents section with this version using much softer colors:
...
const customComponents = {
  ...
  p: ({ node, children, ...props }: any) => (
    <p className="mb-4 leading-relaxed text-gray-300 drop-shadow" {...props}>
      {children}
      <InlineImageRenderer images={images} />
    </p>
  ),
  h1: ({ node, ...props }: any) => <h1 className="text-3xl font-serif mb-6 text-gray-200 drop-shadow-lg font-bold" {...props} />,
  h2: ({ node, ...props }: any) => <h2 className="text-2xl font-serif mb-4 text-gray-200 drop-shadow-lg font-bold" {...props} />,
  h3: ({ node, ...props }: any) => <h3 className="text-xl font-serif mb-3 text-gray-200 drop-shadow-lg font-semibold" {...props} />,
  h4: ({ node, ...props }: any) => <h4 className="text-lg font-serif mb-2 text-gray-200 drop-shadow-lg font-semibold" {...props} />,
  ul: ({ node, ...props }: any) => <ul className="list-disc pl-6 mb-4 text-gray-300" {...props} />,
  ol: ({ node, ...props }: any) => <ol className="list-decimal pl-6 mb-4 text-gray-300" {...props} />,
  li: ({ node, ...props }: any) => <li className="mb-2 text-gray-300 leading-relaxed" {...props} />,
  blockquote: ({ node, ...props }: any) => (
    <blockquote className="border-l-4 border-white/30 pl-4 italic my-6 text-gray-400 bg-black/20 p-4 rounded-r-lg shadow-sm" {...props} />
  ),
  a: ({ node, ...props }: any) => (
    <a className="text-blue-300 hover:text-blue-200 underline underline-offset-2 transition-colors" {...props} />
  ),
  em: ({ node, ...props }: any) => <em className="italic text-gray-300 font-medium" {...props} />,
  strong: ({ node, ...props }: any) => <strong className="font-bold text-gray-200 drop-shadow" {...props} />,
  code: ({ node, inline, className, ...props }: any) => {
    ...
    return inline ? (
      <code className="bg-black/30 px-2 py-1 rounded text-sm text-gray-100 font-mono border border-white/10" {...props} />
    ) : (
      <div className="my-4">
        <pre className="bg-black/30 p-4 rounded-lg text-sm overflow-x-auto text-gray-100 font-mono border border-white/20">
          <code {...props} />
        </pre>
      </div>
    );
  ...
  pre: ({ node, children, ...props }: any) => (
    <div className="my-4">
      <pre className="bg-black/30 p-4 rounded-lg text-sm overflow-x-auto text-gray-100 font-mono border border-white/20" {...props}>
        {children}
      </pre>
    </div>
  ),
  table: ({ node, ...props }: any) => (
    <div className="overflow-x-auto my-6 rounded-lg border border-white/30">
      <table className="min-w-full" {...props} />
    </div>
  ),
  thead: ({ node, ...props }: any) => <thead className="bg-black/20" {...props} />,
  tbody: ({ node, ...props }: any) => <tbody className="divide-y divide-white/20" {...props} />,
  tr: ({ node, ...props }: any) => <tr className="hover:bg-black/10 transition-colors" {...props} />,
  th: ({ node, ...props }: any) => (
    <th className="border-b border-white/30 px-4 py-3 text-left text-gray-200 font-semibold bg-black/30" {...props} />
  ),
  td: ({ node, ...props }: any) => (
    <td className="px-4 py-3 text-gray-300 border-b border-white/20" {...props} />
  ),
  hr: ({ node, ...props }: any) => <hr className="my-8 border-white/30 border-t-2" {...props} />,
  img: ({ node, ...props }: any) => (
    <EnhancedImage
      {...props}
  ...
  return (
    <div ref={containerRef} className={`formal-logic-content ${className}`}>
      {/* Toggle Controls */}
      {showToggle && (
        <div className="flex items-center justify-between mb-4 p-3 bg-black/20 rounded-lg border border-white/20">
          <div className="flex items-center gap-2">
            <span className="text-sm text-gray-200">View Mode:</span>
      ...
      {viewMode === 'raw' ? (
        <div className="relative">
          <pre className={cn(
            ...
            "bg-black/40 p-4 rounded-lg border border-white/20",
            "text-gray-100 overflow-x-auto max-h-96 overflow-y-auto",
