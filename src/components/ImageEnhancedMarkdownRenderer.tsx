        ...
        <div className="flex gap-2">
          ...
          {import.meta.env.DEV && retryCount < fallbackPaths.length - 1 && (
            <Button
              variant="outline"
              size="sm"
              className="bg-primary/10 border-primary/30 text-primary hover:bg-primary/15"
...
const customComponents = {
  ...
  h1: ({ node, ...props }: any) => <h1 className="text-3xl font-serif mb-6 text-foreground/90  font-bold" {...props} />,
  h2: ({ node, ...props }: any) => <h2 className="text-2xl font-serif mb-4 text-foreground/90  font-bold" {...props} />,
  h3: ({ node, ...props }: any) => <h3 className="text-xl font-serif mb-3 text-foreground/90  font-semibold" {...props} />,
  h4: ({ node, ...props }: any) => <h4 className="text-lg font-serif mb-2 text-foreground/90  font-semibold" {...props} />,
  ul: ({ node, ...props }: any) => <ul className="list-disc pl-6 mb-4 text-muted-foreground" {...props} />,
  ol: ({ node, ...props }: any) => <ol className="list-decimal pl-6 mb-4 text-muted-foreground" {...props} />,
  li: ({ node, ...props }: any) => <li className="mb-2 text-muted-foreground leading-relaxed" {...props} />,
  blockquote: ({ node, ...props }: any) => (
    <blockquote className="border-l-4 border-border pl-4 italic my-6 text-muted-foreground/80 bg-secondary/40 p-4 rounded-r-lg shadow-sm" {...props} />
  ),
  a: ({ node, ...props }: any) => (
    <a className="text-primary hover:text-primary/80 underline underline-offset-2 decoration-1 transition-colors" {...props} />
  ),
  em: ({ node, ...props }: any) => <em className="italic text-muted-foreground font-medium" {...props} />,
  strong: ({ node, ...props }: any) => <strong className="font-bold text-foreground/90" {...props} />,
  ...
  th: ({ node, ...props }: any) => (
    <th className="border-b border-border px-4 py-3 text-left text-foreground/90 font-semibold bg-card/60" {...props} />
  ),
  td: ({ node, ...props }: any) => (
