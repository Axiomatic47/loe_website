...
const CountryList = ({
  ...
  return (
    <div className="bg-card/60 p-4 rounded-lg border border-border h-[600px] overflow-y-auto mt-4">
      <div className="sticky top-0 bg-card p-2 -m-2 mb-2 border-b border-border space-y-2">
        <input
          type="text"
          placeholder="Search countries..."
          value={searchTerm}
          onChange={(e) => setSearchTerm(e.target.value)}
          className="w-full bg-card text-foreground border border-border rounded p-2 text-sm"
        />
        <div className="flex gap-2">
          <button
            ...
            className={cn(
              ...
              sortBy === "sgm" ? "bg-secondary text-foreground" : "bg-card/60 text-muted-foreground/80"
            )}
          >
            Sort by SGM
          </button>
          <button
            ...
            className={cn(
              "flex-1 text-xs py-1 px-2 rounded",
              sortBy === "name" ? "bg-secondary text-foreground" : "bg-card/60 text-muted-foreground/80"
            )}
          >
            Sort by Name
          </button>
          ...
          return (
            ...
            >
              <div className="flex justify-between items-center">
                <span className="text-foreground">{country.name || country.country}</span>
                  ...
                  {trend !== null && (
                    <span className="text-xs">
                      {trend > 0.1 ? (
                        ...
                        <Minus className="h-3 w-3 text-muted-foreground/80" />
                      )}
                    </span>
                  )}
                  <span className={`${getColorClass(country.gscs || country.sgm)} px-2 rounded text-sm text-foreground`}>
                    {(country.gscs || country.sgm).toFixed(1)}
                  </span>
                </div>
              </div>
              <div className="text-sm text-muted-foreground/80">{country.category}</div>
            </div>
          );
        })}
      </div>
...
const CountryDetail = ({
  ...
  if (!country) return (
    <div className="bg-card/80 p-6 rounded-lg border border-border mt-4 text-center py-12">
      <p className="text-muted-foreground">Select a country to view detailed analysis</p>
    </div>
  );
  ...
  return (
    <div className="bg-card/80 p-6 rounded-lg border border-border mt-4 space-y-6">
      {/* Header */}
      <div>
        <h3 className="text-2xl font-medium text-foreground mb-1">{country.name || country.country}</h3>
        <p className={`text-sm ${categoryColor}`}>{categoryLabel}</p>
      </div>

      {/* SGM Overview */}
      <div>
        <h4 className="text-lg text-foreground mb-3">Supremacism Governance Model (SGM)</h4>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="bg-card/60 p-3 rounded">
            <div className="text-sm text-muted-foreground/80">Current SGM</div>
            <div className={`text-2xl font-bold ${getTextColor(sgmScore)}`}>
              {sgmScore.toFixed(2)}
            </div>
            {sgmTrend !== null && (
              <div className={`text-xs flex items-center gap-1 mt-1 ${sgmTrend > 0 ? 'text-red-400' : sgmTrend < 0 ? 'text-green-400' : 'text-muted-foreground/80'}`}>
          ...
          <div className="bg-card/60 p-3 rounded">
            <div className="text-sm text-muted-foreground/80">Domestic (SRS-D)</div>
            <div className={`text-xl font-medium ${getTextColor(country.srsD || 0)}`}>
              {country.srsD?.toFixed(1) || "N/A"}
            </div>
          </div>
          <div className="bg-card/60 p-3 rounded">
            <div className="text-sm text-muted-foreground/80">International (SRS-I)</div>
            <div className={`text-xl font-medium ${getTextColor(country.srsI || 0)}`}>
              {country.srsI?.toFixed(1) || "N/A"}
            </div>
          </div>
          <div className="bg-card/60 p-3 rounded">
            <div className="text-sm text-muted-foreground/80">GSCS</div>
      ...
      {latestMonth && (
        <div>
          <h4 className="text-lg text-foreground mb-3">
            ...
            <span className="text-xs text-muted-foreground/70 ml-2">Source: GDELT BigQuery</span>
          </h4>
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            <div className="bg-card/60 p-3 rounded">
              <div className="text-sm text-muted-foreground/80">Goldstein Scale</div>
              ...
              <div className="text-xs text-muted-foreground/70">-10 (conflict) to +10 (cooperation)</div>
            </div>
            <div className="bg-card/60 p-3 rounded">
              <div className="text-sm text-muted-foreground/80">Total Events</div>
              <div className="text-xl font-medium text-foreground">
                {latestMonth.events?.toLocaleString() || 0}
              </div>
              <div className="text-xs text-muted-foreground/70">GDELT coded events</div>
            </div>
            <div className="bg-card/60 p-3 rounded">
              <div className="text-sm text-muted-foreground/80">Conflict Ratio</div>
              <div className="text-xl font-medium text-yellow-400">
                {latestMonth.conflict_ratio ? (latestMonth.conflict_ratio * 100).toFixed(1) + '%' : 'N/A'}
              </div>
              <div className="text-xs text-muted-foreground/70">% of adversarial events</div>
            </div>
            <div className="bg-card/60 p-3 rounded">
              <div className="text-sm text-muted-foreground/80">Conflict Events</div>
              <div className="text-xl font-medium text-orange-400">
                {latestMonth.conflict_events?.toLocaleString() || 0}
              </div>
              <div className="text-xs text-muted-foreground/70">QuadClass 3 & 4</div>
            </div>
            <div className="bg-card/60 p-3 rounded">
              <div className="text-sm text-muted-foreground/80">Violence Events</div>
              <div className="text-xl font-medium text-red-400">
                {latestMonth.violence_events?.toLocaleString() || 0}
              </div>
              <div className="text-xs text-muted-foreground/70">CAMEO 18, 19, 20</div>
            </div>
            <div className="bg-card/60 p-3 rounded">
              <div className="text-sm text-muted-foreground/80">Protest Events</div>
              <div className="text-xl font-medium text-purple-400">
                {latestMonth.protest_events?.toLocaleString() || 0}
              </div>
              <div className="text-xs text-muted-foreground/70">CAMEO 14</div>
            </div>
          </div>
        </div>
      )}
      ...
      {countryTimeline.length > 0 && (
        <div>
          <h4 className="text-lg text-foreground mb-3">12-Month Historical Data</h4>
              ...
              <tbody>
                {countryTimeline.slice().reverse().map((month, idx) => (
                  <tr key={month.month} className={cn(
                    ...
                    idx === 0 ? "bg-secondary/60" : "hover:bg-secondary/60"
                  )}>
                    <td className="py-2 px-2 text-foreground">
                      {month.month}
                      {idx === 0 && <span className="text-xs text-muted-foreground/70 ml-1">(latest)</span>}
                    </td>
      ...
      {country.sti !== undefined && (
        <div>
          <h4 className="text-lg text-foreground mb-2">Stability & Transition Index</h4>
          <div className="bg-card/60 p-3 rounded">
            <div className="flex justify-between mb-1">
              <span className="text-sm text-muted-foreground/80">STI Score: {country.sti}</span>
              <span className="text-sm text-muted-foreground/80">{getSTILabel(country.sti)}</span>
            </div>
      ...
      <div>
        <h4 className="text-lg text-foreground mb-2">Analysis Summary</h4>
        <div className="bg-card/60 p-4 rounded">
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
      <div className="md:col-span-1">
        <h2 className="text-xl text-foreground mb-2">Countries ({countries.length})</h2>
        <CountryList
          countries={countries}
      ...
      <div className="md:col-span-2">
        <h2 className="text-xl text-foreground mb-2">Detailed Analysis</h2>
        <CountryDetail
