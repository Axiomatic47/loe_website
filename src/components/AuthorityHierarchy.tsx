      ...
      <div className="space-y-3">
        ...
        <div className="relative">
          {chain.map((step, index) => (
            <div key={index} className="relative pl-8 pb-6 last:pb-0">
              ...
              {/* Node */}
              <div className="absolute left-0 top-0 w-6 h-6 rounded-full bg-black/60 border-2 border-white/30 flex items-center justify-center">
                <span className="text-xs font-bold text-white">{step.tier}</span>
              </div>
        ...
        {constraint && (
          <div className="mt-4 p-3 bg-amber-900/20 border border-amber-500/30 rounded-lg">
            ...
            <div className="text-white text-sm">{constraint}</div>
          </div>
        )}
        ...
        {position.constitutional_source && position.constitutional_source.length > 0 && (
          <div className="mt-4">
            <div className="text-gray-400 text-xs font-semibold mb-2">CONSTITUTIONAL SOURCES</div>
        ...
        {position.relationships && (
          <div className="mt-4 space-y-2">
            {position.relationships.subordinate_to && position.relationships.subordinate_to.length > 0 && (
              <div>
                <div className="text-gray-400 text-xs font-semibold mb-1">REPORTS TO</div>
                <div className="flex flex-wrap gap-2">
            ...
            {position.relationships.superior_to && position.relationships.superior_to.length > 0 && (
              <div>
                <div className="text-gray-400 text-xs font-semibold mb-1">SUPERVISES</div>
                <div className="flex flex-wrap gap-2">
  ...
  const renderGlobalHierarchy = () => {
    ...
    return (
      <div className="space-y-4">
        <h3 className="text-lg font-semibold text-white mb-4">Constitutional Authority Hierarchy</h3>
          ...
          <div key={tierId} className="relative">
            ...
            <div className={cn(
              ...
              <div className="flex items-center gap-3 mb-2">
                <span className="w-8 h-8 rounded-full bg-black/40 flex items-center justify-center text-sm font-bold">
                  {tierId.replace('tier_', '')}
                </span>
                <span className="font-semibold text-white">{tier.name}</span>
              </div>
              <p className="text-sm text-gray-300 ml-11">{tier.description}</p>
              <p className="text-xs text-gray-500 ml-11 mt-1 italic">{tier.constraint}</p>
            </div>
          </div>
        ))}
      </div>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <h2 className="text-2xl font-bold text-white">Authority Hierarchy</h2>
      ...
      <div className="flex flex-col md:flex-row gap-4">
        ...
        <div className="flex-1">
          <label className="block text-sm text-gray-400 mb-2">Filter by Branch</label>
          <select
        ...
        {viewMode === 'comparison' && (
          <div className="flex-1">
            <label className="block text-sm text-gray-400 mb-2">Compare With</label>
            <select
                ...
                return (
                  <div key={tier} className="space-y-2">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className="text-gray-500 text-xs">({tierPositions.length})</span>
                    </div>
                      ...
                      {tierPositions.map(pos => (
                        ...
                        >
                          ...
                          <div>
                            ...
                            <div className="text-xs text-gray-500">{pos.status}</div>
                          </div>
                        </button>
                      ))}
                    </div>
