// src/components/MathJaxMarkdownRenderer.tsx - CLEAN VERSION - No conflicts
...
import { Code, Eye, Copy, Check, X, AlertCircle } from 'lucide-react';
...
// Simple image component - no external dependencies
const SimpleImage: React.FC<{
  ...
  // Simple path resolution - no external utilities
...
const MathJaxMarkdownRenderer: React.FC<MathJaxMarkdownProps> = ({
  ...
  // CLEAN: Simple MathJax processing using ONLY global MathJax
  useEffect(() => {
    if (containerRef.current && viewMode === 'rendered') {
      // Wait a bit for DOM to be ready
      const timer = setTimeout(() => {
        // Use the global MathJax from index.html
        if (typeof window !== 'undefined' && window.MathJax) {
          // Clear any existing MathJax processing first
          const existingMath = containerRef.current?.querySelectorAll('.MathJax');
          existingMath?.forEach(el => el.remove());

          // Process with global MathJax
          if (window.MathJax.typesetPromise) {
            window.MathJax.typesetPromise([containerRef.current])
              .then(() => {
                console.log('✅ Global MathJax processed content successfully');
              })
              .catch((err: any) => {
          ...
          console.log('⏳ Waiting for global MathJax to load...');
        }
      }, 150);
  ...
  // Clean markdown components - fixed DOM nesting issues
  const customComponents = {
    // Fix DOM nesting: don't put block elements inside paragraphs
